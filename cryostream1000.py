...
# Controls Cryostream 1000 (Oxford Cryosystems)
class Cryostream1000:

    # Constructor
    def __init__(self, ip):

        # Stores IP of the Cryostream 1000
        self._ip = ip

        # Default Port for TCP Communication
        self._port = 10304   

        # Default interruption time 1 second
        # Still not updated on the device
        self._interruptionTime = 1

        # Folder containing two Cryostream 1000 files:
        ...
        # oxCryoDataFolderPath = "/home/staff/gabriel/Development/sourcecode/Cryostream/cryostream1000-Python2/oxcryoData/"
    ...
    # Method to print a Cryostream 1000 object
    # Pending Implementation
    def __str__(self):
        s = ""
        s += "Model: Cryostream 1000"
        return s
    ...
    # Populates a Dictionary called _oxCryoProperties with All Cryostream 1000 Properties
    # ID, Name
    # 1002, Min Temp
    # 1003, Max Temp
    # 1031, Total Hours
    ...
    def _parseBinaryStatusPacket(self, binaryStatusPacket):
        ...
        # Remove last element of the list (Zero)
        binaryStatusList.pop()

        # Sanity Check - Currently at 1148 parameters
        if (len(binaryStatusList)%4) != 0:
            print("The number of elements on the status package from Cryostream 1000 is")
        ...
        for i in range(0, len(binaryStatusList), 4):
            ...
            # OxCryo Cryostream 1000 Ethernet Communications Documentation
    ...
    # Builds dictionary with the last status of the Cryostream 1000
    ...
    def _updateStatus(self):
        ...
        self._lastBinaryStatusPacket = self._getBinaryStatusPacket()

        # print("Parsing Cryostream 1000 binary status packet...")
    ...
    def _binarizeCommand(self, cmdList):
        ...
        # Cryostream 1000 requires 7 bytes as input to generate a binary command.
        if len(cmdList) != 7:
            print("Error in binarizeListOfCommands(): In order to create a binary command to the Cryojet 1000, it is necessary to have a list of exactly 7 integers between 0 and 255, in order to generate a binary command of size 7 bytes.")
            sys.exit(1)
    ...
    def _launchCommand(self, command, p1, p2):
        ...
        # print("Sending Binary Command to the Cryostream 1000.")
    ...
    # Function to capture binary status packets from Cryostream 1000.
    # This device broadcasts a status packet every second on the network on port 10304.
    def _getBinaryStatusPacket(self):
        ...
        # Checks if the device is on the network
        online = self.pingIP(ip)

        if online == False:

            print("Device " + ip + " is not online on the network!")
            print("We cannot ping the device on the network.")
            print("Possible Solutions: ")
            print("- Wait a few seconds to rerun the software, Cryostream closed the connection.")
            print("- Check if the Cryostream 1000 is connected to the network.")
            print("- Check if you can ping the Cryostream 1000 from your terminal.")
        ...
        binaryStatusPacket = s.recv(8192)

        # Close the socket
        s.close()

        # Returns raw binary data from the device
        return binaryStatusPacket

    # Receives a binary command and sets to the Cryostream 1000
    def _submitBinaryCommand(self, bin):
        ...
        # Sets a smaller send buffer size of 7 bytes only.
        s.setsockopt(socket.SOL_SOCKET, socket.SO_SNDBUF, 7)  
    ...
    # Returns which mode the Cryostream 1000 is curretly running:
    # ID #1053  
    # [0] - Initializing
    ...
    # Returns Annealing Interruption Time
    # ID #???
    def getFlowInterruptionTime(self):
        attribute = "FC Interrupt time"
        return self._lastStatus[attribute]
    ...
    # Returns the IP of the machine set. (Cryostream 1000).
    # ID #N/A
    def getIP(self):
        return self._ip

    # Returns 10304 (TCP), Constant Port Which Cryostream 1000 uses.
    # ID #N/A
    def getPort(self):
        return self._port
    ...
    # Time for which the flow will be interrupted before calling Interrupt flow now (Command ID #121)
    # Command ID: 120
    def setFlowInterruptionTime(self, time):
        code = self._commandBook["Set flow interrupt time"]
        self._launchCommand(code,time,time)

    # Interrupt Flow Now (Annealing)
    # Command ID: 121 (No parameters)
    def interruptFlowNow(self):
        code = self._commandBook["Interrupt flow now"]
        self._launchCommand(code,0,0)
    ...
    def setTurboModeWithConfirmation(self, desiredMode, maxRetries = 10):
        ...
        while (retries < maxRetries):
            ...
            # Cryostream 1000 sometimes sets Turbo Mode as 2 or 3, which means Turbo Mode is On.
    ...
    # Time for which the flow will be interrupted before calling Interrupt flow now (Command ID #121)
    # Command ID: 120
    def setFlowInterruptionTimeWithConfirmation(self, interruptionTime, maxRetries = 10):
        ...
        # Saves flow interruption time before command is sent
        before = self.getFlowInterruptionTime()

        #Loop until the desired mode is set or max retries reached
        while (retries < maxRetries):

            print("Set Flow Interruption Time: Attempt " + str(retries+1) + " out of " + str(maxRetries) + ".")

            self._launchCommand(code,interruptionTime,interruptionTime)
            ...
            # Getting run mode information
            after = self.getFlowInterruptionTime()

            #print("Time before: ", before, type(before), "Time after: ", after, type(after))

            if(after == interruptionTime):
                return True

            # Increment Retry Count
            retries +=1

        print("It was not possible to set the Interruption time.")
        print("Please, try again!")
        return False    
    ...
    def shutdownAndGetReady(self):
        ...
        if(result == False):
            ...
            print("Cryostream 1000 is not in Ready Mode!")
            print("Attempting to bring Cryostream 1000 to Ready Mode!")
    ...
    # Puts the Cryostream 1000 in Ready State
    ...
    def getReadySetTargetTemperatureAndGo(self, temperature):
        ...
        if(isReadyOrRunning == False):
            print("Cryostream 1000 is not in Ready or Running Mode!")
            print("Attempting to bring Cryostream 1000 to Ready Mode!")
        ...
        # === If the Universe is not aligned ===
            print("It was not possible to bring Cryostream 1000 to Ready Mode!")
    ...
    # Set Flow Interruption Time General Function
    def setFlowInterruptionTimeGeneral(self, interruptionTime):

        isInterruptionTimeFloat = self._isFloat(interruptionTime)

        if(isInterruptionTimeFloat == True):
            # Converting to Float
            interruptionTime = float(interruptionTime)
        else:
            print("You interruption time does not correspond to a float.")
            return False

        #print(interruptionTime, type(interruptionTime))
        # If new flow interruption time is in range, we send the command to set it
        if(10.0 <= interruptionTime <= 600.0 and isInterruptionTimeFloat == True):

            # Sets the interruption time
            # We multiply by 10, to convert to ds.
            result = self.setFlowInterruptionTimeWithConfirmation(interruptionTime)

            # If Flow interruption time was updated sucessfully
            if (result == True):
                # We update the interruption time local variable on our class
                self._interruptionTime = interruptionTime
                print("Flow interruption time set successfully to " + str(self._interruptionTime/10.0) + "(s).")
                return True
            else:
                # Failed
                print("Sorry, Not your lucky day. Failed to set flow interruption time.")
                print("Flow interruption time still at " + str(self._interruptionTime/10.0) + "(s).")
                return False
        else:
            print("Interruption time is not in the range of 01 to 60 seconds.")
            print("Flow interruption time still at " + str(self._interruptionTime/10.0) + "(s).")
            return False
    ...
    # Things to do, before initializing terminal.
    def terminal_init(self):

        # Sets default flow interruption time (annealing) for 1 second = 10ds
        self.setFlowInterruptionTime(10)
    ...
    def terminal_drawLogo(self):
        ...
        print("                                     \033[1mCryostream 1000 Ed.\033[0m")        
        print("")       
    ...
    def terminal_drawMenu(self):
        ...
        # Menu Itself
        print("")
        print("Cryostream 1000 (IP " + self.getIP() + ":" + str(self.getPort()) + ") [\033[1m" + onlineStatus + "\033[0m]:")
        ...
        print("\033[1m[5]\033[0m Set Flow Interruption Time.")
        print("\033[1m[6]\033[0m Interrupt Flow for " + str(self.getFlowInterruptionTime()/10.0) + "(s).")
        print("\033[1m[7]\033[0m Set Autofill Mode.")
    ...
    # Terminal based interface for the Cryostream 1000
    ...
    def terminal_displayMenu(self):
        ...
        while True:
            ...
            #print("interruptTime", self.getFlowInterruptionTime())
            # Draws Menu
            ...
            # Case 01 - Updates Run Mode that appears on the opening screen
                ...
                print("Updating Run Mode (Status).")
            ...
            # Case 05 - Set Flow Interruption Time
            elif choice == 5:

                self.terminal_setFlowInterruptionTime()


            # Case 06 - Interrupt Flow
            elif choice == 6:

                self.terminal_interruptFlowNow()


            # Case 07 - Set Auto Fill Mode
            elif choice == 7:
    ...
    # Shows Cryostream 1000 information on screen
    # Case 00 - Display info related to the device
    def terminal_displayInfo(self):
        ...
        # Cryostream 1000 Port - 10304 TCP
        ...
        interruptionTime  = str(self.getFlowInterruptionTime()/10.0)
    ...
    # Set Flow Interruption Time
    # Case 05 - Set Flow Interruption Time
    def terminal_setFlowInterruptionTime(self):

        # Current Flow Interruption Time
        print("Current flow interruption time at " + str(self.getFlowInterruptionTime()/10.0) + "(s).")
        
        # New Flow Interruption Time
        interruptionTime = raw_input("Please, type an interruption time between 1.0(s) and 60.0(s): ")

        # Is the interruption time a float?
        isInterruptionTimeFloat = self._isFloat(interruptionTime)

        # if it is a float, we can convert
        if(isInterruptionTimeFloat == True):
            # Converting to Float
            interruptionTime = float(interruptionTime) * 10 #Converts from s to ds
        else:
            # Failed, not right.
            print("You interruption time does not correspond to a float.")
            return False
        ...
        # Device Limits
        minTime = 1.0   #In ds, which means 00.1 seconds
        maxTime = 600.0 #In ds, which means 60.0 seconds

        # Check if the time is within the allowed interval
        if not (minTime <= interruptionTime <= maxTime):
            print("Error: Time should be between [" + str(minTime) + "," + str(maxTime) + "]. You provided " + str(interruptionTime) + ".")
            return False

        # Defines the amount of seconds that flow will be interrupted
        # When interruptFlowNow() is called
        self.setFlowInterruptionTimeGeneral(interruptionTime)



    # Interrupts the flow, right now, for a pre-set number of seconds.
    # Case 06 - Interrupt FLow Now
    def terminal_interruptFlowNow(self):

        #Stop, Get Ready and Cool
        self.interruptFlowNow()



    # Sets Autofill Mode
    # Case 07 - Manual, Auto or Scheduled
